...
function M.setup(config)
	...
	vim.api.nvim_exec(
		[[augroup Cursorword
        au!
        au CursorMoved                   * lua Cursorword.auto_highlight()
        au InsertEnter,TermEnter,QuitPre * lua Cursorword.auto_unhighlight()

        au FileType TelescopePrompt let b:cursorword_disable=v:true
      augroup END]],
		false
	)

	-- Create highlighting
	vim.api.nvim_exec([[hi default Cursorword term=underline cterm=underline gui=underline]], false)
end
...
--- everything is setup in |Cursorword.setup|.
...
--- is setup in |Cursorword.setup|.
function M.auto_unhighlight()
	-- Stop any possible previous delayed highlighting
	H.timer:stop()
	H.unhighlight()
end
...
function H.is_disabled()
	return vim.g.cursorword_disable == true or vim.b.cursorword_disable == true
end
...
function H.highlight()
	...
	if H.window_matches[win_id] == nil then
		...
		local match_id = vim.fn.matchadd("Cursorword", curpattern, -1)
